...
#[derive(GraphQLQuery)]
#[graphql(
    schema_path = "src/server/github/schema.graphql",
    query_path = "src/server/github/get_sponsors.graphql",
    ...
    Ok(response_body
        ...
        .filter_map(|edge| {
            ...
            let privacy_level = match &node.privacy_level {
                get_sponsors::SponsorshipPrivacy::PRIVATE => "private".to_owned(),
                get_sponsors::SponsorshipPrivacy::PUBLIC => "public".to_owned(),
                get_sponsors::SponsorshipPrivacy::Other(x) => x.to_owned(),
            };
...
pub(crate) async fn refresh_db(app_state: &AppState) -> Result<()> {
